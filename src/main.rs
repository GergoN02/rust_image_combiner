...
impl FloatingImage {
    fn new(width: u32, height: u32, name: String) -> Self {
        let buffer_cap = height * width * 4;
        let buffer = Vec::with_capacity(buffer_cap.try_into().unwrap());
        FloatingImage {
            width,
            height,
            data: buffer,
...
fn main() -> Result<(), ImageDataErrors> {
    let args = Args::new();
    let (image_1, image_format_1) = find_img_from_path(args.image_1);
    let (image_2, image_format_2) = find_img_from_path(args.image_2);
